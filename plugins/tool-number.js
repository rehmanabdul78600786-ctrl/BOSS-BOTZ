...
const config = require("../config"); // for PREFIX
...
cmd({
    ...
    try {
        if (!args || args.length < 1) return reply(
        ...
        const { data } = await axios.get(
            ...
            { timeout: 10000, validateStatus: s => s === 200 }
        );
        ...
        reply(`${errorMessage}\n\n🔑 Remember: ${config.PREFIX || "."}otpbox <number>`);
    }
});
...
cmd({
    ...
    try {
        ...
        if (!data?.result) return reply("❌ Couldn't fetch country list.");
...
cmd({
    ...
    try {
        if (!args[0] || !args[0].startsWith("+")) return reply(
        ...
        if (!data?.result || !Array.isArray(data.result)) return reply("⚠ No OTP messages found for this number");
