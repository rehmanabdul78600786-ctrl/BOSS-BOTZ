...
cmd({
  ...
  react: '✏️',
  desc: "Add or change caption of media/document/audio/video",
  category: "utility",
  filename: __filename
}, async (client, message, match, { from }) => {
  try {
    // Check if the user replied to a media message
    if (!message.quoted) {
      return await client.sendMessage(from, {
        text: "*🍁 Please reply to a media message (image/video/document/audio) to add a caption!*\n\n*Usage:*\n- Reply to media with .caption [your text]\n- Or just .caption [text] to add caption to previous media"
      }, { quoted: message });
    }

    const quotedMsg = message.quoted;
    
    // Validate the quoted message
    ...
    if (!newCaption) {
      return await client.sendMessage(from, {
        text: "❌ Please provide a caption text"
      }, { quoted: message });
    }
    ...
    // Attach media according to its type
    switch (mtype) {
      case "imageMessage":
    ...
    // Send the new message with caption
