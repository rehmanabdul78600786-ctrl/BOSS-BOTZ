...
cmd({
    ...
    try {
        ...
        // Main API
        const mainApi = `https://api.popcat.xyz/v2/blur?image=${encodeURIComponent(imageUrl)}`;
        let response;

        try {
            response = await axios.get(mainApi, { responseType: "arraybuffer" });
        } catch (err) {
            // Fallback API — ensures command always works
            const fallbackApi = `https://vihangayt.me/filter/blur?url=${encodeURIComponent(imageUrl)}`;
            try {
                response = await axios.get(fallbackApi, { responseType: "arraybuffer" });
            } catch (fallbackErr) {
                throw new Error("Both blur APIs failed.");
            }
        }
