...
cmd({
    ...
    try {
        ...
        // Main API
        const mainApi = `https://api.popcat.xyz/v2/ad?image=${encodeURIComponent(imageUrl)}`;
        let response;
        ...
        try {
            ...
            // Backup API
            const fallbackApi = `https://vihangayt.me/filter/ad?url=${encodeURIComponent(imageUrl)}`;
            try {
                response = await axios.get(fallbackApi, { responseType: "arraybuffer" });
            } catch (fallbackErr) {
                throw new Error("Both AD APIs failed.");
            }
        }
