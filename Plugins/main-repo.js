...
cmd({
    ...
    try {
        ...
        if (!match) return reply("⚠️ Invalid GitHub URL format.");
        ...
        if (!response.ok) {
            return reply(`⚠️ GitHub API error: ${response.status}`);
        }
...
> *ﮩ٨ـﮩﮩ٨ـ 𝑩𝑶𝑺𝑺ﮩ٨ـﮩﮩ٨ـ🖤*
        ...
        reply("❌ Something went wrong while fetching repository info.");
    }
});
